...
func (s *server) Connect(msg *goChittyChat.Post, srv goChittyChat.ChatService_ConnectServer) error {
	...
	s.Broadcast(&goChittyChat.Post{Id: userData.id, Lamport: msg.Lamport, Message: userMsg})
	srv.Send(&goChittyChat.Post{Id: userData.id, Lamport: msg.Lamport, Message: userMsg})
...
func (s *server) Disconnect(_ context.Context, in *goChittyChat.Post) (out *goChittyChat.Empty, err error) {
	usr := s.users[in.Id]
...
func (s *server) Messages(srv goChittyChat.ChatService_MessagesServer) error {
	for {
		...
		// We omitted error handling for this particular case, as this was not a requirement for the assignment.
